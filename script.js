...
//checks the game stat to choose the appropriate image and then set the player width and height accordinly
...
// Initialize levels -> platforms, monsters and goal
function init_level1() {
	var level = {};
	level.name = "Level 1";
	init_level_defaults(level);

	//staring position default
	level.player_start_x = 20;
	level.player_start_y = 260;

	//create the platfroms.
	...
	//create the monsters
    var monster_data =[
		// [x_start, y, width, height, min_x, max_x, move_x, image]
		[350, 360, 20, 20, 60, 470, -1.0, "vlad"],
		[400, 360, 20, 20, 60, 470, 0.8, "henrietta"],
...
//adding transforms to the context to make it easier to draw the player and monsters facing left or right
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        if (!t.found) {
            ...
            for (var ival = 0; ival < t.value_images.length; ival++) {
                var curr_percent = (t.value_max > 0) ? 100 * t.value / t.value_max : 100;
                if (t.value_images[ival].percent >= curr_percent) {
                    index = ival;
                    break;
                }
            }
            var img = t.value_images[index] && t.value_images[index].img;
            if (img && img.complete) {
                set_transform(ctx, t);
...
function draw_player(ctx) {
	...
	if (sprite.img && sprite.img.complete) {
...
function draw_status(ctx) {
    ...
    var level = _levels[_game.current_level];
    if (level.player_has_key && _game.img_key.complete) {
...
//transition state for while a timer counts down; when the timer hits 0, start the next level
...
//If the 2 objects overlap, return a [dx, dy, dir] array contianing the x, y
